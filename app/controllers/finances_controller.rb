class FinancesController < ApplicationController
  ...
  def index
    ...
    @expenses = Expense.joins("left join `types` t ON t.id = type_id").select("expenses.description,min(expenses.date) as mindate,max(expenses.date) as maxdate,t.name,value,sum(expenses.value) as totalvalue").group('expenses.value, expenses.description')
